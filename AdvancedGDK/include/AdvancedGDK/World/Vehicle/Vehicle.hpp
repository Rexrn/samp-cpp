...
enum class VelocityUnit : std::uint8_t
{
	GameDefault,
	KMPH,
	MPH
...
public:
	...
	void setSpeed(float const speed_);
	...
	float getSpeed(const VelocityUnit unit_ = VelocityUnit::KMPH) const;
	...
	/// <summary>
	/// Returns SAMP internal vehicle handle or -1 if not spawned.
	/// </summary>
	...
	std::int32_t getModel() const {
		return m_modelIndex;
	}

	/// <summary>
	/// Returns the world vehicle is bound to.
	/// </summary>
	...
	/// <summary>
	/// Returns the interior vehicle is bound to.
	/// </summary>
	/// <returns>Vehicle's interior.</returns>
	...
	std::int32_t getFirstColor() const {
		return m_firstColor;
	}
	...
	std::int32_t getSecondColor() const {
...
protected:
	...
	std::int32_t						m_handle;			// Handle for SAMP object. -1 if not spawned
	std::int32_t						m_modelIndex;		// Vehicle Model ID
	std::int32_t						m_world;			// World that vehicle is bound to 
	std::int32_t						m_interior;			// Interior that vehicle is bound to
	math::Vector3f						m_location;			// Vehicle's location.			It is NOT updated at real time. Its used when vehicle is not spawned.
	float								m_angle;			// Vehicle's angle.				It is NOT updated at real time. Its used when vehicle is not spawned.
...
/// <summary>
/// Class for managing static vehicles.
/// </summary>
