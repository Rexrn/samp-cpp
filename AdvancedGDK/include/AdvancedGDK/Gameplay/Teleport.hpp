...
{
	/// <summary>
	/// Class used as a namespace "Teleport"
	/// </summary>
	class Teleport
	{
	public:
		constexpr static std::int32_t cxprInvalid		= -1;
		constexpr static std::int32_t cxprNoChange		= -2;

		constexpr static std::int32_t cxprEvery			= -1;
		constexpr static std::int32_t cxprAllButDefault	= -2;
		...
		/// </summary>
		constexpr Teleport()
			: facingAngle{ 0 }, world{ cxprNoChange }, interior{ cxprNoChange }
		{
		}
		...
		constexpr Teleport(Vector3 const location_, float const facingAngle_)
			: location{ location_ }, facingAngle{ facingAngle_ }, world{ cxprNoChange }, interior{ cxprNoChange }
		{
		}
		...
		constexpr Teleport(Vector3 const location_, float const facingAngle_, std::int32_t const world_, std::int32_t const interior_)
		...
		constexpr static Teleport absolute(Vector3 const location_, float const facingAngle_, std::int32_t const world_, std::int32_t const interior_)
		{
		...
		/// <param name="facingAngle_">The facing angle.</param>
		/// <returns>Constructed teleport</returns>
		constexpr static Teleport relative(Vector3 const location_, float const facingAngle_)
		{
			return Teleport(location_, facingAngle_, cxprNoChange, cxprNoChange);
		}
		...
		/// <param name="interior_">The interior.</param>
		/// <returns>Constructed teleport</returns>
		constexpr static Teleport relativeW(Vector3 const location_, float const facingAngle_, std::int32_t const interior_)
		{
			return Teleport(location_, facingAngle_, cxprNoChange, interior_);
		}
		...
		/// <param name="world_">The world.</param>
		/// <returns>Constructed teleport</returns>
		constexpr static Teleport relativeI(Vector3 const location_, float const facingAngle_, std::int32_t const world_)
		{
			return Teleport(location_, facingAngle_, world_, cxprNoChange);
		}
	...
	{
		///////////////////////////////////////////// Main cities
		static constexpr Teleport GroveStreet{ Vector3(2495.3755f, -1669.4906f, 13.5163f), 0 };
		static constexpr Teleport SanFierro{ Vector3(-1986.7297f, 884.7870f, 45.1206f), 0 };
		static constexpr Teleport LasVenturas{ Vector3(2140.6675f, 993.1867f, 10.5248f), 0 };
