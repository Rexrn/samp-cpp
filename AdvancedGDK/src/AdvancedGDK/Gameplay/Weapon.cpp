...
#include <AdvancedGDK/Gameplay/Weapon.hpp>
...
namespace agdk
{
	...
	Weapon::Type Weapon::findByName(std::string_view const name_)
	{
		...
		cpName = StringHelper::toLower(cpName);
		for (std::int32_t i = Fist; i <= Parachute; i++)
		{
			if ( cpName == StringHelper::toLower( Weapon::getNameFn(static_cast<Weapon::Type>(i)) ) )
	...
	Weapon::Type Weapon::findBestMatch(std::string_view const name_)
	{
		...
		if (result == Weapon::Invalid)
		{
			...
			if (result == Weapon::Invalid)
			{
				...
				std::string compareName = StringHelper::toLower(cpName);

				for (std::int32_t i = Fist; i <= Parachute; i++)
				{
					// Search for max sequence in the string.
					auto tempScore = StringHelper::maxSequenceLength(compareName,
