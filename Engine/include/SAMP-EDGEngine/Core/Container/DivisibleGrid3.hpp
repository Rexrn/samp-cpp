...
private:
	...
	math::Vector3size computeArrayIndices(math::Vector3f const & location_) const
	{
		...
		math::Vector3d const relativeLocation = location_.convert<double>() - (this->getCenter().template convert<double>() - halfExtent);
		...
		using VecType = math::Vector3size;

		return VecType
			{
				VecType::ValueType(std::floor(relativeLocation.x / (childHalfExtent.x * 2.0))),
				VecType::ValueType(std::floor(relativeLocation.y / (childHalfExtent.y * 2.0))),
				VecType::ValueType(std::floor(relativeLocation.z / (childHalfExtent.z * 2.0)))
			};
	}
