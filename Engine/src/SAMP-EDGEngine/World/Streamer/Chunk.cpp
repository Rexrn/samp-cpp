...
{
	std::size_t const	countOfObjects	= m_globalObjects.size() + m_universalObjects.size() + m_personalObjects.size() + m_vehicles.size();
	auto const			colorValue		= std::size_t( std::min(std::size_t(255), countOfObjects) );
